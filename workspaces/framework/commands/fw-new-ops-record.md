...
queue (FEAT-195), the shape from the plugin's `templates/records/ops-record.md`
(a project's `.claude/templates/records/` overrides), and the record lands in the
root `operations/open/` queue (ADR-009 D2 as amended by TASK-213). The queue
scaffold is created automatically on first use, from
`templates/queues/operations/` (a project's `.claude/templates/queues/`
overrides). The prefix carries the kind; the folder carries the status. Customer
ticket ids are a cross-reference field, never our key.
...
1. **Run the script** — the create gate:
   ...
   errors (bad kind, missing template), report its message verbatim and stop —
   do not create anything by hand.
