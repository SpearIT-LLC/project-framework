...
│       └── research/
│
├── thoughts/                     # ← FROM starter template
│   ├── work/
│   │   ├── backlog/
...
| **Live framework** | `framework/thoughts/` | **EXCLUDED** (framework's own history) |
| **Starter template** | `templates/starter/` | Root docs with `{{PLACEHOLDER}}` tokens |
| **Starter template** | `templates/starter/` | `thoughts/` structure with `.limit` files |
...
| `framework/thoughts/` | **No** | Framework's own work history |
...
| **`thoughts/` → `project-hub/` rename** | Affects many docs; do separately then build on new structure |

---

## Related Work Items

- **TECH-xxx** (to be created): Rename `thoughts/` to `project-hub/`
