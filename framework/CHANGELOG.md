...
- **TECH-156: Command reference brought up to date** — Rewrote `framework/docs/ref/framework-commands.md` to accurately document all 11 `/fw-*` commands (it previously covered only 5). Added reference sections for `/fw-next-id`, `/fw-session-history`, `/fw-roadmap`, `/fw-topic-index`, `/fw-swarm`, and `/fw-release`; fixed the stale `/fw-wip-check` name (now `/fw-wip`) and removed the non-existent `/fw-archive` example.
