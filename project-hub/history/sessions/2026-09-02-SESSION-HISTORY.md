...
# Late Afternoon — Re-scoping the Survivors, and Rehoming C6

**Continuation.** The afternoon closed TASK-218 with three threads explicitly left open.
This block closes two of them: the BUG-181/FEAT-175 re-scopes (decided at lunch, unwritten
until now) and C6's homelessness. The third — writing findings into the four held cards
(SPIKE-178, DECISION-162, TECH-096, DECISION-110) — remains open.

---

## Summary (late afternoon)

BUG-181 and FEAT-175 were rewritten against `workspaces/framework/`, keeping their IDs so
the reasoning that earned them stays attached. Verifying each against the new build made
both **worse** than the re-scope decision had assumed — in each case the new build turned
out to have *less* than the old one, not a different version of it. Then C6 got a home:
**TASK-219** for the working detail, **D1b** in the roadmap for the ranking.

---

## Work Completed

### BUG-181 → re-scoped: "The Collaboration Contract Does Not Reach a Generated Project"

Renamed on disk (`BUG-181-starter-claude-md-missing-collaboration-contract.md` →
`BUG-181-contract-not-delivered-to-generated-project.md`) because the old filename named a
dead file.

**Verification changed the severity.** In the old pipeline a derived project at least
*received* the contract as an unreferenced file (`framework/CLAUDE.md`) — the July bug was
a missing **pointer**. In the new build there is neither file nor pointer:

| Checked 2026-09-02 | Result |
|---|---|
| `.claude/framework-contract.md` | exists — 4,807 bytes, authored 2026-07-22 |
| Anything in `workspaces/framework/` referencing it | **none** — the SoT is inert |
| `/fw-init` | **does not exist** (5 commands ship; it is not among them) |
| …but `README.md:11` | already promises *"structure … is generated by `/fw-init`"* |
| `fw-new-workspace.sh` generating a `CLAUDE.md` | no — and no `CLAUDE*` in `templates/` |
| Region markers in the new build's own `CLAUDE.md` | **absent** |

ADR-007's D1/D3/D4 still govern the contract's *shape* — ADR-009 changed only the delivery
mechanism (build step → `/fw-init`). Two design questions were left for pre-implementation
rather than decided unilaterally: **where the SoT lives** once it ships inside the plugin,
and **whether the new build dogfoods the region markers** (ADR-007 D3's compose-starter /
verify-root asymmetry has no obvious analogue when the framework *is* the plugin).

Old-build detail kept under a **Superseded History** heading with an explicit *"Dead
artifacts — do not resurrect"* list, so the July investigation stays readable as reasoning
without reading as a plan.

### FEAT-175 → re-scoped: the board create gate

The new build has **four** create gates — workspace, ops record, contact, kb domain — and
**none for board items**. Also verified: **no type SoT exists in the new build at all**
(no `work-item-types.txt` or equivalent under `workspaces/framework/`), so the accepted set
is currently both unenforced *and* unstated there.

Sequenced with the ADR-009 D5 crossover, because `fw-next-id.sh` refuses the `kanban`
namespace until the board crosses over — a create gate for an inactive namespace would be
speculative.

**Plugin-parity scope dropped entirely.** The framework *is* the plugin, so there is one
copy by construction; SPIKE-178, FEAT-179 and TECH-169 no longer gate this card. That is
the same "dissolved by construction" pattern the morning identified, now applied.

**The design carried over untouched** because it was never pipeline-specific: strict-script
/ lenient-AI, prefix-normalization validated against 74 real-world type strings with zero
false positives, rejection-as-education, and the scoped-ID hazard (a naive grep returned
`202`, matching `ROADMAP-2026-02-04.md` as `ROADMAP-202`, where the correct scoped scan
returned `178` — 25 IDs silently burned on first run).

**A new dependency surfaced while writing it:** work-item templates do not exist in the new
build, and their conventions are precisely the C6 cards. FEAT-175 is therefore *blocked* by
C6 — which is what forced the next piece of work.

### TASK-219 created + roadmap D1b — C6 gets a home

Gary: *"That's why I wanted the updated roadmap but I'm fine with it's own card."* Both,
because they do different jobs.

- **TASK-219** holds the working detail — the sixteen grouped by what they block, each with
  what the new build needs it for.
- **Roadmap D1b** holds the ranking — rank 2, immediately behind the spine.

---

## Decisions Made (late afternoon)

### 12. Re-scope means card surgery, not replacement

Both cards keep their original IDs, their creation dates, and their accumulated reasoning.
Only the target changes. The alternative — archive the old, file a new one — would have
produced a card with no history and silently discarded the July audit that explains *why*
the fix is a composer rather than a copy.

**Consequence:** each card now carries a re-scope banner at the top and a superseded-history
section at the bottom. The banner tells a reader which parts to trust; the history tells
them why the dead parts died.

### 13. C6 needs both a card and a roadmap entry — they are not redundant

The roadmap answers *when and why does this matter relative to everything else*; the card
answers *what exactly is in it and what is the next action*. C6 as a single row in D8's
"what is NOT archived" table could not carry sixteen cards' worth of grouping, and TASK-218
(in `done/`, bound for a release archive) could not carry a ranking.

**The failure this avoids:** the C6 analysis was, until now, reachable only through a
completed card. At the next release it would have moved to
`history/releases/…` and become effectively invisible — a live blocker filed under history.

### 14. C6 is held to a mechanism test, not a documentation test

Every one of the sixteen must end as a convention **plus** its enforcement — a template
field, a script check, a hook — or an explicit statement that it is one of the rules that
cannot be mechanized. A convention decided and written only in prose is ADR-008 Root 2, the
exact failure mode this framework was rebuilt to avoid.

### 15. Group 1 is settled as a set, before any template is authored

FEAT-021 (numbering), TECH-082 (parent/child), TECH-041 (supporting files), TECH-027
(cross-references) and TECH-033 (status vs. folder) interlock: numbering constrains
sub-items, which constrain cross-references, which constrain the template's fields.
Deciding them one at a time would produce a template that has to be rewritten after each.

**And the ordering matters for a second reason:** the work-item template is FEAT-175's
prerequisite. Author it before these five are settled and it encodes guesses that a create
gate then bakes into filenames that live forever.

---

## The Finding Worth Carrying Forward

**`fw-move.sh:6` already treats "child items" as grouping that moves with its parent — and
nothing in the repo defines what a child item is.**

The engine is ahead of its conventions. That is the same shape as the ADR-008 failures: a
mechanism exists, the rule it implements was never written, and the two drift silently
until something breaks. It is the single clearest argument for D1b's rank.

---

## Files Created (late afternoon)

- `project-hub/work/todo/TASK-219-board-conventions-for-the-new-build.md` — owns the
  sixteen, grouped by what they block, held to the mechanism test

## Files Modified (late afternoon)

- `project-hub/work/todo/FEAT-175-fw-new-deterministic-create-gate.md` — rewritten for the
  ADR-009 build
- `project-hub/planning/ROADMAP-DELIVERABLES.md` — D1b inserted at rank 2; **Sequence block
  rewritten** (it still showed D5 "jumps the queue" and D8 "one bulk move", both superseded
  earlier today); header basis line refreshed to 84 open + 26 archived, verified against
  the filesystem
- `project-hub/work/done/TASK-218-deprecate-old-framework-cards.md` — C6 said *"Yes — 13"*
  while listing 16; corrected at the source and pointed at TASK-219

## Files Moved (late afternoon)

- `BUG-181-starter-claude-md-missing-collaboration-contract.md` ->
  `BUG-181-contract-not-delivered-to-generated-project.md` (`git mv`)

---

## Current State (end of day)

**Board: 71 backlog · 12 todo · 0 doing · 7 done · 1 blocked · 26 deprecated**
(84 open + 26 archived; next ID **219** — now consumed by TASK-219, so next is 220)

### In done/ (awaiting release)
- BUG-207, BUG-208, BUG-212, FEAT-193, FEAT-195, TASK-206, TASK-218 — 7 items, under the
  10-item release threshold
...
- **D1 remains the recommended start** (TASK-213/214/216 + BUG-215), with **D1b/TASK-219
  immediately behind it** and pairing with the D5 crossover.
- **Group 1 of TASK-219 is the highest-leverage single session available** — five
  interlocking conventions that unblock the work-item template, which unblocks FEAT-175.
- **Still open from the afternoon:** four held cards (SPIKE-178, DECISION-162, TECH-096,
  DECISION-110) need their findings written *into* the cards. Those answers currently live
  only in TASK-218 — the same stranding problem TASK-219 just solved for C6.
- **Still unresolved from 2026-09-01:** does the root move land before or after the D5
  board crossover? Three sessions have now deferred this. It gates D1's sequencing and
  should be decided at D1's pre-implementation review rather than deferred a fourth time.
- **`/fw-init` is promised but unbuilt.** `workspaces/framework/README.md:11` describes it
  as the mechanism that generates repo structure. BUG-181 depends on it. It has no card of
  its own — worth filing before something else assumes it exists.

---

**Last Updated:** 2026-09-02 (late afternoon)
